...
It looks at the resting orders on the opposite side of the book for
that asset (buy orders look at asks, sell orders look at bids), sorted
by best price first, then earliest placed.
It fills against them one at a time — a market order takes whatever
price is resting; a limit order only matches at prices at least as good
as its own limit.
Filling stops when: the incoming order is fully filled, the book runs
...
thing).
Any unfilled remainder of a limit order is inserted into the
